#!/usr/bin/env bash
# ✍️ Quilly — Content Director
# Instagram content, Reel scripts, B-Roll plans, captions.
set -euo pipefail
...
banner() {
  printf '\033[0;33m'
  printf '╔══════════════════════════════════════════╗\n'
  printf '║  ✍️  QUILLY  •  Content Director         ║\n'
...
case "$CMD" in
  ...
  comment)
    ...
    CARD_ID=$(trello_find_card "$TITLE")
    trello_comment_card "$CARD_ID" "✍️ Quilly: $TEXT"
