#!/usr/bin/env bash
# 💰 Larry — Sales & Revenue
# DM outreach, proposals, follow-ups, pipeline management. Hannover market.
set -euo pipefail
...
case "$CMD" in
  ...
  comment)
    ...
    CARD_ID=$(trello_find_card "$TITLE")
    trello_comment_card "$CARD_ID" "💰 Larry: $TEXT"
